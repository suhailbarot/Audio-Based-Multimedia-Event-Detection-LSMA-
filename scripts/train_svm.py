...
if __name__ == '__main__':
    ...
    X_train = []
    Y_train = []
    # svm_classifier = SVC(probability=True,kernel='rbf',class_weight={1:5},gamma='scale')
    # svm_classifier = SVC(gamma='scale',kernel="rbf")
    # svm_classifier = LogisticRegression()

    #Try to define a max depth????
    # Try 450 cluster size????
    svm_classifier = RandomForestClassifier(n_estimators=150)
    ...
    # file_list = train_files_list.readlines() + val_files_list.readlines()
    # shuffle(merged_file_list)
    for line in merged_file_list.readlines():
