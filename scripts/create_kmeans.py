...
if __name__ == '__main__':
    ...
    kmeans_model = sys.argv[1]; file_list = open(sys.argv[3])
    cluster_num = int(sys.argv[2])


    # load the kmeans model
    data = []
    mfcc_files = set(glob.glob('mfcc/*.csv'))
    kmeans = cPickle.load(open(kmeans_model+'.pkl',"rb"))
    i=0
    for line in tqdm(file_list.readlines()):
        if "mfcc/"+line.strip()+'.mfcc.csv' not in mfcc_files:
            ...
            reader = csv.reader(open("mfcc/"+line.strip()+'.mfcc.csv'))
            for line1 in reader:
                temp.append(np.array(map(float,line1[0].split(';'))))
            feature_vector = kmeans.predict(temp)
